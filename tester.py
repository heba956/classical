...
class PathTester:
    ...
    def __init__(self, cones: List[Cone], car_pose: CarPose, track_width: float = 3.0):
        self.cones = cones
        self.car_pose = car_pose
        self.track_width = track_width

    def run(self) -> Path2D:
        planner = PathPlanning(self.car_pose, self.cones, track_width=self.track_width)
    ...
    def _plot_scene(self, path: Path2D) -> None:
        ...
        # Optionally save the plot to a file when running headless or for CI.
        # Set environment variable SAVE_PLOT=1 to enable saving to 'out.png'.
        try:
